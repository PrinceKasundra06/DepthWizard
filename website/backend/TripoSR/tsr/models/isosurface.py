...
def marching_cubes(volume, isovalue):
    # PyMCubes expects numpy array, returns numpy arrays
    vol_np = volume.cpu().numpy()
    v, t = mcubes.marching_cubes(vol_np, isovalue)
    return torch.from_numpy(v.copy()).float(), torch.from_numpy(t.copy()).long()
